...
impl TableInfo {
    ...
    pub fn new(data: &[u8], valid_bitvec: u64) -> Result<Self> {
        ...
        for table_id in TableId::iter() {
            ...
            // Tolerate truncated table row count data — malware/obfuscators may
            // declare tables in the valid mask but truncate the row count area.
            // dnlib handles this by silently stopping; we do the same.
