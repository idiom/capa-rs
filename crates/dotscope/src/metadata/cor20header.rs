...
impl Cor20Header {
    ...
    pub fn read(data: &[u8]) -> Result<Cor20Header> {
        ...
        if flags & !VALID_FLAGS != 0 {
            // Warn but don't reject — malware/obfuscators commonly set undefined CLR flag bits.
            // The .NET runtime itself ignores unknown flags (dnlib does the same).
            #[cfg(feature = "logging")]
            log::warn!(
                "Cor20Header: CLR flags 0x{:08X} contain undefined bits, ignoring [ECMA-335 §II.25.3.3]",
                flags
            );
        }
