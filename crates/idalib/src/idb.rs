...
use crate::ffi::insn::{
    decode, idalib_generate_disasm_line, idalib_print_insn_mnem, idalib_print_operand,
};
...
impl IDB {
    ...
    /// Get the mnemonic text for the instruction at the given address.
    pub fn mnemonic_at(&self, ea: Address) -> Option<String> {
        let s = unsafe { idalib_print_insn_mnem(ea.into()) };
        if s.is_empty() { None } else { Some(s) }
    }

    /// Generate the full disassembly line at the given address.
    pub fn disasm_line_at(&self, ea: Address, gen_flags: i32) -> String {
        unsafe { idalib_generate_disasm_line(ea.into(), gen_flags.into()) }
    }

    /// Print a single operand of the instruction at the given address.
    pub fn print_operand_at(&self, ea: Address, n: usize) -> Option<String> {
        let s = unsafe { idalib_print_operand(ea.into(), (n as i32).into()) };
        if s.is_empty() { None } else { Some(s) }
    }
